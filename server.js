...
const handleSubtractionInput = (a, b, res) => {
    if(calc.sanitizeNumbers(a,b)) {
        res.json({'result': calc.sub(calc.sanitizeNumbers(a, b)) });
    } else {
	const errorMsg2 = `number1 and number2 must to be numberish.
		          but number1 was: ${a} and number2 was: ${b} `;

        logger.log('error', 'sanitizeNumbers failed', {number1: a, number2: b});
        res.status(404).send(errorMsg2);
    }
};



const handleDivisionInput = (a, b, res) => {
    if(calc.sanitizeNumbers(a,b)) {
        res.json({'result': calc.divide(calc.sanitizeNumbers(a, b)).toFixed(3) });
    } else {
	const errorMsg3 = `number1 and number2 must to be numberish.
		          but number1 was: ${a} and number2 was: ${b} `;

        logger.log('error', 'sanitizeNumbers failed', {number1: a, number2: b});
        res.status(404).send(errorMsg3);
    }
};
...
app.post('/calculator/sub', (req, res) => {
    logger.log('info', 'Request arrived in the add endpoint', {requestBody: req.body});
    handleSubtractionInput(req.body.number1, req.body.number2, res);
});


app.post('/calculator/div', (req, res) => {
    logger.log('info', 'Request arrived in the add endpoint', {requestBody: req.body});
    handleDivisionInput(req.body.number1, req.body.number2, res);
});


app.post('/calculator/multiply', (req, res) => {
    logger.log('info', 'Request arrived in the add endpoint', {requestBody: req.body});
    handleMultiplyInput(req.body.number1, req.body.number2, res);
});
